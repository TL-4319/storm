...
phd = GGIW_RFS.GGIW_PHD(clustering_obj=clustering,extract_threshold=0.2,merge_threshold=12,**RFS_base_args)
phd.gating_on = False
...
truth_model = GGIWMixture(alphas=[5.0, 5.0], 
            betas=[1/30.0, 1/30.0],
            means=[np.array([50, 15, -1, 0]).reshape((4, 1)),np.array([10, 5, 0, 2]).reshape((4, 1))],
            covariances=[np.diag([0,0,0,0]),np.diag([0,0,0,0])],
            IWdofs=[30.0, 30.0],
            IWshapes=[np.array([[70, 25],[25, 70]]),np.array([[70, 25],[25, 70]])])
...
for kk, t in enumerate(time[:-1]):
    ...
    for ii in range(len(truth_model._distributions)):
        temp = (truth_model._distributions[ii].sample_measurements(xy_inds=[0,1]))
        if ii == 0:
            x = temp[0]
            y = temp[1] 
        else:
